...
Membership site for "The November Window" ($47/mo). Next.js App Router + Supabase + Stripe + Resend + Mux on Vercel.
...
- **Access rule lives in one place:** `lib/access.ts` (`admin || comp_access || active/trialing sub`). Change it there only.
...
## Money flow
Landing CTA → `GET /api/checkout` (Stripe Checkout, subscription) → webhook `checkout.session.completed` → create/find Supabase user → upsert `subscriptions` → Resend welcome email containing an admin-generated magic link → `/auth/callback` → `/course`.
